...
import { ChevronLeft, CheckCircle2, ShieldCheck, Mail, Save } from "lucide-react";
import Link from "next/link";
import { Textarea } from "@/components/ui/textarea";

export default function OfferCreationPage({ params }: { params: { id: string } }) {
  // Mock API tender based on ID
  const tender = {
    id: params.id || "AU-89342",
    buyerType: "IT-Dienstleister",
    location: "München (80331)",
    expiresIn: "2 Tage 14 Std.",
    radius: "Bundesweit",
    vehicles: [
      { quantity: 5, brand: "Audi", model: "A4 Avant S-Line", specs: "2.0 TDI 204 PS, Mythosschwarz Metallic, Matrix LED, Assistenzpaket Tour", listPrice: 62450 }
    ],
  ...
  // State Management for Offer Form
  ...
  const totalBasePrice = tender.vehicles.reduce((acc, v) => acc + (v.listPrice * v.quantity), 0);
        ...
        <div className="container mx-auto max-w-7xl px-4 md:px-8 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <h1 className="font-bold text-lg md:text-xl flex items-center gap-3">
              Angebot abgeben <Badge className="bg-white/20 text-blue-200 border-none px-2 rounded-md font-mono">{tender.id}</Badge>
            </h1>
          </div>
          <div className="hidden md:flex items-center text-sm font-medium text-blue-200">
            Endet in: <span className="text-amber-400 ml-2 font-bold bg-amber-400/10 px-2 py-0.5 rounded-md">{tender.expiresIn}</span>
          </div>
        </div>
      </div>
          ...
          <div className="lg:w-[45%] lg:sticky lg:top-[88px] h-fit flex flex-col gap-6">
            <div className="bg-white border border-slate-200 rounded-3xl p-6 shadow-sm">
              ...
              <ul className="space-y-4 text-slate-600">
                <li className="flex gap-3">
                  ...
                  <p><strong className="text-navy-900">Anonymität wahren:</strong> Versuchen Sie nicht, Kontaktinformationen im Angebot zu platzieren. Der Kunde meldet sich bei Ihnen.</p>
                </li>
                <li className="flex gap-3">
                  ...
                  <p><strong className="text-navy-900">Endpreisgarantie:</strong> Kalkulieren Sie ehrlich inklusive aller Nebenkosten. Verdeckte Gebühren führen zum Ban.</p>
                </li>
                <li className="flex gap-3">
                  <div className="w-6 h-6 rounded-full bg-blue-50 text-blue-600 flex items-center justify-center font-bold text-xs shrink-0 mt-0.5">3</div>
                  <p><strong className="text-navy-900">Alternativen:</strong> Der Kunde akzeptiert explizit <b>{tender.acceptAlternatives.higherTrim ? 'höhere Ausstattungen' : 'keine höheren Ausstattungen'}</b>. Andere Farben sind <b>{tender.acceptAlternatives.color ? 'erlaubt' : 'nicht erlaubt'}</b>.</p>
                </li>
              </ul>
            </div>
            ...
            <div className="bg-white border border-slate-200 rounded-3xl p-6 shadow-sm">
              ...
              <div className="grid grid-cols-2 gap-y-4">
                <div>
                  <div className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-1">Branche</div>
                  <div className="font-semibold text-navy-900">{tender.buyerType}</div>
                </div>
                <div>
                  <div className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-1">Standort</div>
                  <div className="font-semibold text-navy-900">{tender.location}</div>
                </div>
                <div>
                  <div className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-1">Radius</div>
                  <div className="font-semibold text-navy-900">{tender.radius}</div>
                </div>
                <div>
                  <div className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-1">Verträge</div>
                  <div className="font-semibold text-navy-900">{tender.fleetDiscount ? 'Großkundenvertrag' : 'Keine vorhanden'}</div>
                </div>
              </div>
            </div>
            ...
            <div className="bg-slate-100 border border-slate-200 rounded-3xl p-6">
              ...
              {tender.vehicles.map((v, i) => (
                <div key={i} className="bg-white rounded-2xl p-5 border border-slate-200 shadow-sm relative overflow-hidden">
                  ...
                  <div className="font-bold text-navy-950 text-xl">{v.brand} {v.model}</div>
                  <div className="text-slate-500 text-sm mt-2 mb-4 pr-12 leading-relaxed">{v.specs}</div>
                  <div className="bg-slate-50 p-3 rounded-xl border border-slate-100 text-sm flex justify-between items-center">
                    ...
                    <span className="font-bold text-navy-900">{v.listPrice.toLocaleString('de-DE')} €</span>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="lg:w-[55%] flex flex-col gap-6">
            ...
            <div className="bg-white border text-left border-slate-200 rounded-3xl p-6 md:p-10 shadow-sm">
              ...
              <div className="flex items-start gap-4 mb-6">
                ...
                <div>
                  ...
                  <p className="text-slate-500">Wenn Sie alternative Fahrzeuge anbieten (z.B. andere Farbe, Tageszulassung, sofort verfügbar), schalten Sie diesen Punkt aus.</p>
                </div>
              </div>
              ...
              {!exactMatch && (
                <div className="animate-in fade-in slide-in-from-top-4">
                  ...
                  <Textarea 
                    placeholder="z.B. Fahrzeug ist weiß statt schwarz, dafür sofort verfügbar und mit erweitertem Assistenzpaket..."
                    value={deviationDesc}
            ...
            <div className="bg-white border border-slate-200 rounded-3xl p-6 md:p-10 shadow-sm">
              ...
              <div className="space-y-10">
                {tender.requestedTypes.includes("Kauf") && (
                ...
                {tender.requestedTypes.some(t => t.includes("Leasing")) && (
                  <div className="bg-white border-2 border-dashed border-slate-300 p-6 sm:p-8 rounded-3xl">
                    <div className="flex items-center justify-between mb-6">
                      ...
                      <Badge variant="outline">Anfrage: 36M / 15.000km</Badge>
                    </div>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        ...
                        <div className="space-y-3">
                          ...
                          <div className="h-14 bg-slate-100 rounded-xl border border-transparent flex items-center px-4 font-bold text-navy-600 text-lg">
                            {((parseFloat(leasingRate) / (totalBasePrice / 1.19)) * 100).toFixed(2)} % 
